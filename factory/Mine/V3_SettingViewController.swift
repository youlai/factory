//
//  V3_SettingViewController.swift
//  MasterWorker
//
...
class V3_SettingViewController: UIViewController {
    @IBOutlet weak var uv_back: UIView!
    @IBOutlet weak var uv_jd: UIView!
    ...
    override func viewDidLayoutSubviews() {
        ...
        gradientLayer.frame = btn_logout.bounds
        self.btn_logout.layer.insertSublayer(gradientLayer, at: 0)
        self.btn_logout.clipsToBounds=true
    }
    override func viewDidLoad() {
        super.viewDidLoad()
        uv_back.addOnClickListener(target: self, action: #selector(back))
        uv_jd.addOnClickListener(target: self, action: #selector(toJd))
    ...
    //MARK:接单设置
    @objc func toJd(){
//        self.navigationController?.pushViewController(V3_JdSettingViewController(), animated: true)
    }
    //MARK:修改密码
    @objc func pwd(){
    ...
    //MARK:检查更新
    @objc func update(){
        VersionManager.init(appleId: "1509650055",auto: false)
    }
    //MARK:退出账号
    @objc func logout(){
