...
class ChooseCommonView: UIView,UITableViewDelegate,UITableViewDataSource {
    
    
    @IBOutlet weak var tableview: UITableView!
    var brandList=[ProdCommon]()
    var selectIndex:IndexPath = IndexPath(row: 0, section: 0)
    //选中分类闭包回调
    var brandSelect: ((_ brandOfxgy: ProdCommon) -> Void)? = nil
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel!.text="\(item.BrandName ?? "")-\(item.ProdModel ?? "")"
