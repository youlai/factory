...
class ChooseProdCategoryView: UIView,UITableViewDelegate,UITableViewDataSource {
    
    
    @IBOutlet weak var tableview: UITableView!
    var brandList=[ProdCategory]()
    var selectIndex:IndexPath = IndexPath(row: 0, section: 0)
    //选中分类闭包回调
    var brandSelect: ((_ brandOfxgy: ProdCategory) -> Void)? = nil
