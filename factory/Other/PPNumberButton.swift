...
@IBDesignable open class PPNumberButton: UIView {
    ...
    @objc fileprivate func touchDown(_ button: UIButton) {
        ...
        let ti=TimeInterval(CGFloat.greatestFiniteMagnitude)
        if button == decreaseBtn {
            timer = Timer.scheduledTimer(timeInterval: ti, target: self, selector: #selector(self.decrease), userInfo: nil, repeats: true)
        } else {
            timer = Timer.scheduledTimer(timeInterval: ti, target: self, selector: #selector(self.increase), userInfo: nil, repeats: true)
        }
        timer.fire()
    }
