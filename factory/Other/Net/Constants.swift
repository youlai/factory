 ...
 * "6"://已完结
* */
//let NewWorkerGetOrderList=BASE_URL2+"Order/NewWorkerGetOrderList"//MARK:v3师傅端获取工单列表NewFactoryGetOrderList
let NewFactoryGetOrderList=BASE_URL2+"Order/NewFactoryGetOrderList"//MARK:v3工厂端获取工单列表
/**
* 各工单数量
* var Count1 = 0;//待处理数量
* var Count2 = 0;//待预约数量
...
let FactoryNavigationBarNumber=BASE_URL2+"Order/FactoryNavigationBarNumber"//MARK:工厂端端获取工单列表选项卡数量
...
let GettokenbyUserid=BASE_URL2+"account/GettokenbyUserid"//MARK:获取token
let LoginOn=BASE_URL2+"Account/LoginOn"//MARK:登录
let WxRegister=BASE_URL2+"Account/WxRegister"//MARK:微信登录
let WxReg=BASE_URL2+"Account/WxReg"//MARK:绑定手机号
let Reg=BASE_URL2+"Account/Reg"//MARK:注册
let GetChildAccountByParentUserID=BASE_URL2+"Account/GetChildAccountByParentUserID"//MARK:获取子账号
let CancelChildAccount=BASE_URL2+"Account/CancelChildAccount"//MARK:注销子账号
let ForgetPassword=BASE_URL2+"Account/ForgetPassword"//MARK:忘记密码ForgetPassword
let GetUserInfoList=BASE_URL2+"Account/GetUserInfoList"//MARK:西瓜鱼用户信息
let ToBepresent=BASE_URL2+"Account/ToBepresent"//MARK:查询提现中

let DepositRechargeList=BASE_URL2+"Account/DepositRechargeList"//MARK:充值保证金记录
let GetDepositWithDrawList=BASE_URL2+"Account/GetDepositWithDrawList"//MARK:提取保证金记录

let RechargeRecord=BASE_URL2+"Account/RechargeRecord"//MARK:充值记录
let FreezingAmount=BASE_URL2+"Account/FreezingAmount"//MARK:冻结记录
let WorkOrderTime=BASE_URL2+"Account/WorkOrderTime"//MARK:工单记录

let UploadAvator=BASE_URL2+"Upload/UploadAvator"//MARK:上传头像
let GetCode=BASE_URL2+"Message/Send"//MARK:发送验证码 type注册1  忘记密码2 验证码登录3
let LoginOnMessage=BASE_URL2+"Account/LoginOnMessage"//MARK:验证码登录
let WithDrawDeposit=BASE_URL2+"Account/WithDrawDeposit"//MARK:提取保证金
let GetFactoryCategory=BASE_URL2+"FactoryConfig/GetFactoryCategory"//MARK:获取分类
let GetBrandByCategory=BASE_URL2+"FactoryConfig/GetBrandByCategory"//MARK:获取品牌
let GetFactoryBrandByUserID=BASE_URL2+"FactoryConfig/GetFactoryBrandByUserID"//MARK:获取品牌
let AddFactoryBrand=BASE_URL2+"FactoryConfig/AddFactoryBrand"//MARK:添加品牌
let AddBrandCategory=BASE_URL2+"FactoryConfig/AddBrandCategory"//MARK:添加型号
...
let UpdateOrderState=BASE_URL2+"Order/UpdateOrderState"//MARK:更新工单状态
let ApplyCancelOrder=BASE_URL2+"order/ApplyCancelOrder"//MARK:工单作废
let FactoryComplaint=BASE_URL2+"Order/FactoryComplaint"//MARK:工厂投诉
let FactoryEnsureOrder=BASE_URL2+"Order/FactoryEnsureOrder"//MARK:工厂确认工单 结算
let EnSureOrder=BASE_URL2+"Order/EnSureOrder"//MARK:用户确认订单 结算
let NowPayEnSureOrder=BASE_URL2+"Order/NowPayEnSureOrder"//MARK:用户确认订单 结算
let ApplyCustomService=BASE_URL2+"Order/ApplyCustomService"//MARK:发起质保
let ApproveBeyondMoney=BASE_URL2+"Order/ApproveBeyondMoney"//MARK:审核远程费
let ApproveOrderAccessory=BASE_URL2+"Order/ApproveOrderAccessory"//MARK:审核配件申请
let NewAddOrUpdateExpressNo=BASE_URL2+"Order/NewAddOrUpdateExpressNo"//MARK:工厂添加配件快递信息
let UpdateIsReturnByOrderID=BASE_URL2+"Order/UpdateIsReturnByOrderID"//MARK:旧件是否需要返件
let GetOrderRecordByOrderID=BASE_URL2+"Order/GetOrderRecordByOrderID"//MARK:工单跟踪
let GetExpressInfo=BASE_URL2+"Order/GetExpressInfo"//MARK:物流信息
let GetOrderInfo=BASE_URL2+"Order/GetOrderInfo"//MARK:工单详情
let GetOrderStr=BASE_URL2+"Pay/GetOrderStr"//MARK:支付宝
let GetWXOrderStr=BASE_URL2+"Pay/GetWXOrderStr"//MARK:微信
let MallBalancePay=BASE_URL2+"Mall/MallBalancePay"//MARK:余额
let WXNotifyManual=BASE_URL2+"Pay/WXNotifyManual"//MARK:微信人工回调OutTradeNo
let FAddCon=BASE_URL2+"Account/FAddCon"//MARK:赠送西瓜币
let GetOrderByhmall=BASE_URL2+"Order/GetOrderByhmall"//MARK:获取待支付工单
let GetOrderByhmalluserid=BASE_URL2+"Order/GetOrderByhmalluserid"//MARK:获取待支付工单
let AddorUpdateAccountPayInfo=BASE_URL2+"Account/AddorUpdateAccountPayInfo"//MARK:添加银行卡
let GetAccountPayInfoList=BASE_URL2+"Account/GetAccountPayInfoList"//MARK:获取银行卡
let GetBankNameByCardNo=BASE_URL2+"Account/GetBankNameByCardNo"//MARK:根据银行卡号获取银行名 判断后台是否支持该银行的提现
let IsMallid=BASE_URL2+"Order/IsMallid"//MARK:判断商品订单号是否发起过保内安装
...
let PressWokerAccount=BASE_URL2+"Order/PressWokerAccount"//MARK:催单
let UpdateAccountNickName=BASE_URL2+"Account/UpdateAccountNickName"//MARK:修改昵称
let UpdateEmergencyContact=BASE_URL2+"Account/UpdateEmergencyContact"//MARK:添加紧急联系人
let updateTeamNumber=BASE_URL2+"Account/updateTeamNumber"//MARK:团队人数
let IsOrNoTruck=BASE_URL2+"Account/IsOrNoTruck"//MARK:有无货车
let UpdateSex=BASE_URL2+"Account/UpdateSex"//MARK:修改性别
let AddOpinion=BASE_URL2+"Account/AddOpinion"//MARK:意见反馈
let CoinList=BASE_URL2+"mall/CoinList"//MARK:西瓜币明细
...
let UpdateFactoryAccessorybyFactory=BASE_URL2+"FactoryConfig/UpdateFactoryAccessorybyFactory"//MARK:审核配件 配件价格为0
let ApproveOrderAccessoryByModifyPrice=BASE_URL2+"Order/ApproveOrderAccessoryByModifyPrice"//MARK:审核配件 配件价格不为0
let ApproveOrderAccessoryAndService=BASE_URL2+"Order/ApproveOrderAccessoryAndService"//MARK:审核配件
let GetUserOrderNum=BASE_URL2+"Order/GetUserOrderNum"//MARK:获取已发工单数量
...
let FactoryApplyAuthInfo=BASE_URL2+"Account/FactoryApplyAuthInfo"//MARK:申请实名认证
let GetAccountSkill=BASE_URL2+"Account/GetAccountSkill"//MARK:获取账号技能
let GetmessageBytype=BASE_URL2+"Account/GetmessageBytype"//MARK:获取公司信息
