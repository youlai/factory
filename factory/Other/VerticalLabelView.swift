...
class VerticalLabelView: UIView {
        ...
        willSet {
            ...
            for (index, text) in newValue.enumerated() {
                ...
                if markBtn == nil {
                    ...
                    if markBtn.frame.origin.x + markBtn.frame.size.width + marginY + textWidth + marginX > screenW {
                ...
                labelBtn.backgroundColor = UIColor.lightGray
            ...
            newFrame.size.height = min(totalHeight, screenH - kNavigationBarHeight)
            self.frame = newFrame
            
            self.scrollView.frame = CGRect(x: 0.0, y: 0.0, width: self.frame.width, height: self.frame.height)
            scrollView.contentSize = CGSize.init(width: self.frame.width, height: totalHeight)
        }
    }
    ...
    func textWidth(text: String, font: UIFont) -> CGFloat {
        ...
        let rect = str.boundingRect(with: CGSize.init(width: screenW - 20.0, height: self.labelHeight), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : font], context: nil)
